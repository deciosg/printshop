...
public class DesignController {
	...
	public String list(Model model) {
		model.addAttribute("titleHeader", "Designers");
		model.addAttribute("employees", employeeRepository.findAllByRole(Role.ROLE_DESIGN));
	...
	public String createOrUpdate(@Valid Employee employee, BindingResult bindingResult, Model model) {
		employee.setRole(Role.ROLE_DESIGN);
		if (bindingResult.hasErrors()) {
			return "employee/form";
		}
		employeeRepository.save(employee);
		return "redirect:/designers/" + employee.getId();
	}
